  ...
  return (
    <section className='w-full py-16 px-4 md:px-8 lg:px-16 xl:px-24 bg-white'>
      <div className='max-w-7xl mx-auto flex mt-10 flex-col gap-12'>
        ...
        {/* Metrics Row - 3 individual cards */}
        <div className='flex flex-col md:flex-row justify-between items-stretch gap-6 w-full mt-6 mb-10'>
          {metrics.map((metric, i) => (
            <div
              key={i}
              className='flex flex-1 flex-col items-center justify-center text-center gap-4 py-12 px-6 rounded-[20px]'
              style={{
                background:
                  'linear-gradient(258.79deg, rgba(229, 170, 102, 0.138) -13.56%, rgba(255, 165, 129, 0.138) -13.56%, rgba(221, 161, 108, 0.138) 18.23%, rgba(230, 242, 246, 0.138) 53.64%, rgba(202, 185, 246, 0.138) 91.85%, rgba(112, 48, 177, 0.138) 119.62%)',
              }}
            >
              {/* Large Number */}
              <span
                className='text-5xl font-medium leading-none flex-shrink-0'
                style={{ color: '#262626' }}
              >
                {metric.value}
              </span>
              ...
              {/* Label block */}
              <span className='text-[15px] font-medium leading-snug' style={{ color: '#737373' }}>
                {metric.label}
              </span>
            </div>
          ))}
