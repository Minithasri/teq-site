      ...
      <div className='max-w-[1120px] mx-auto px-5 sm:px-8 text-center'>
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-9'>
          {impacts.map((item, index) => (
            <div
              key={index}
              className='relative bg-white rounded-[8px] p-7 overflow-hidden min-h-[188px] border-2 border-[#F5F5F5] shadow-[0px_8px_22px_rgba(0,0,0,0.08)]'
            >
              ...
              <div className='relative z-10 flex items-center gap-5 md:gap-7'>
                <div className='w-[34px] h-[34px] bg-[#0846AA]/60 rounded-[6px] flex items-center justify-center p-2 shrink-0'>
                  <Image
                    src={item.icon}
                    alt={item.title}
                    width={18}
                ...
                <h3 className='text-[24px] md:text-[26px] font-extrabold tracking-tight text-[#0F1729]/80'>
                  {item.title}
                </h3>
              </div>

              <p className='relative z-10 text-[#0846AA] text-[11px] font-bold mt-5'>
                {item.subtitle}
              </p>
