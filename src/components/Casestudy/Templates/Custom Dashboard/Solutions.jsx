  ...
  return (
    <section className='w-full px-6 md:px-16 lg:px-24 bg-white'>
      <div className='w-full mx-auto'>
        ...
        <div className='mb-8'>
          <h2 className='font-bold mt-12 mb-6' style={{ color: '#2432CB', fontSize: '36px' }}>
            Solution Implemented
          </h2>
          <p className='max-w-4xl' style={{ color: '#6A7181', fontSize: '18px' }}>
        ...
        <div className='relative w-screen left-1/2 right-1/2 -ml-[50vw] -mr-[50vw] p-8 md:p-12 lg:p-24 overflow-hidden'>
          ...
          <div className='max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 relative z-10'>
            {solutions.map((item, index) => (
              <div
                key={index}
                className='bg-white/95 backdrop-blur-sm p-8 rounded-2xl border-[3px] flex flex-col gap-6 hover:shadow-xl transition-all'
                style={{ borderColor: '#2432CB', minHeight: '300px' }}
              >
                <div className='flex items-center gap-4'>
                  <div
                    className='w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 border-2'
                    style={{ borderColor: '#2432CB' }}
                  >
                    <Image src={item.icon} alt={item.title} width={28} height={28} />
                  </div>
                  <h3
                    className='font-bold leading-tight'
                    style={{ color: '#151C28', fontSize: '18px' }}
                  >
                    {item.title}
                  </h3>
                </div>
