  ...
  return (
    <section className='w-full bg-white'>
      <div className='w-full max-w-[1440px] mx-auto px-6 sm:px-10 md:px-16 lg:px-24 py-10 md:py-14'>
        {/* Title */}
        <h2 className='text-2xl md:text-3xl font-bold mb-7 md:mb-8 text-center'>Business Impact</h2>
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5 mb-8 md:mb-10'>
          {metrics.map((item, i) => (
            <div
              key={i}
              className='rounded-2xl p-5 md:p-6 flex flex-col gap-2 text-center border-2 border-[#F5F5F5] shadow-[0px_8px_22px_rgba(0,0,0,0.08)]'
            >
              {/* Metric */}
              <span className='text-2xl md:text-3xl font-bold text-[#0F1729]/80'>{item.value}</span>
              {/* Description */}
